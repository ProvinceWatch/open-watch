...
export const SideBar = ({ handleToggleSidebar, isOpen, children, pt }: SideBarProps) => {
  return (
    <>
      ...
      <div className={`w-80 min-h-screen bg-white p-4 pt-${pt} fixed transform transition-transform duration-300 ${isOpen ? '' : 'translate-x-80'}`} style={{ zIndex: 1, display: 'flex', flexDirection: 'column', height: '100%' }}>
        {children}
      </div>
    </>
  );
